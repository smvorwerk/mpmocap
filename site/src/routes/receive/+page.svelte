		...
		conn.on('data', function (data: unknown) {
			if (data) {
				...
				if (blendshapes && blendshapes.length === 52) {
					...
					if (websocketOpen) {
						websocket.sendWebsocketMessage('/mpmocap/blendshapes', ...blendshapesValue);
					}
				}
				...
				if (leftEye && rightEye && leftEye.length === 4 && rightEye.length === 4) {
					if (websocketOpen) {
						websocket.sendWebsocketMessage('/mpmocap/leftEyeRotation', ...leftEye);
						websocket.sendWebsocketMessage('/mpmocap/rightEyeRotation', ...rightEye);
					}
				}
				...
				if (headRotation && headRotation.length === 4) {
					if (websocketOpen) {
						websocket.sendWebsocketMessage('/mpmocap/headRotation', ...headRotation);
					}
				}
...
<main class="h-full w-full flex flex-col items-center overflow-y-auto">
	<div class="px-4">
		...
		<div
			class="card mt-2 p-2 h-36 min-h-36 w-full max-w-2xl overflow-hidden flex flex-col flex-none"
		>
